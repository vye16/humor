...
class AMASSFitDataset(Dataset):
    ...
    def __init__(
        ...
        if split_by == "dataset" and custom_split is not None:
            split_str = "custom"
        self.amass_dataset = AmassDiscreteDataset(
            split=split_str,
        ...
        if self.return_points:
            ...
            self.male_bm = BodyModel(
                bm_path=male_bm_path, num_betas=16, batch_size=self.seq_len
            ).to(self.device)
            self.female_bm = BodyModel(
                bm_path=female_bm_path, num_betas=16, batch_size=self.seq_len
            ).to(self.device)
    ...
    def __getitem__(self, idx):
        ...
        for k, v in global_data.items():
            ...
            if gt_key == "root_orient" or gt_key == "pose_body":
                ...
                v = rotation_matrix_to_angle_axis(v.reshape((-1, 3, 3))).reshape(
                    (T, -1)
                )
            gt_dict[gt_key] = v
            ...
            if self.root_only:
                for k, v in SMPL_JOINTS.items():
                    if k not in ["leftArm", "rightArm", "head", "neck", "hips"]:
                        observed_dict["joints3d"][:, v, :] = float(
                            "inf"
                        )  # everything but root is not observed
        ...
        if self.return_points:
            ...
            body_gt = cur_bm(
                pose_body=gt_dict["pose_body"].to(self.device),
                pose_hand=None,
            ...
            for t in range(self.seq_len):
                ...
                body_mesh = trimesh.Trimesh(vertices=verts, faces=faces, process=False)
        ...
        if self.make_partial:
            ...
            for k, v in observed_dict.items():
                if k == "joints3d" and self.root_only:
                    continue
                occluded_mask = (
                    observed_dict[k][:, :, 2:3] < self.partial_height
                )  # 0.95 #0.75
        ...
        if "contacts" in gt_dict:
            ...
            full_contacts = torch.zeros((gt_contacts.size(0), len(SMPL_JOINTS))).to(
                gt_contacts
            )
