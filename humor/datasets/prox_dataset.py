        ...
        if not os.path.exists(fpath):
            ...
            for key in param.keys():
                if key in SMPL_NAME_MAP.keys():
                    cur_valid = cur_valid and (
                        torch.sum(
                            torch.logical_not(torch.isfinite(torch.Tensor(param[key])))
                        ).item()
                        == 0
                    )
                    if cur_valid:
                        fit_dict[SMPL_NAME_MAP[key]].append(
                            torch.Tensor(param[key]).reshape((1, -1))
                        )
                    else:
                        fit_dict[SMPL_NAME_MAP[key]].append(
                            torch.zeros(
                                (SMPL_SIZES[SMPL_NAME_MAP[key]]), dtype=torch.float
                            ).reshape((1, -1))
                        )
            if not cur_valid:
...
class ProxDataset(Dataset):
    ...
    def __init__(
        ...
        if self.mask_joints and not self.mask_color:
            print(
                "Must be using color mask in order to mask joints (since detected in RGB)! Will NOT mask joints..."
            )
        ...
        self.fitting_root = (
            os.path.join(self.data_dir, "fittings/mosh")
            if self.quant
            else os.path.join(self.data_dir, "PROXD")
        )
    ...
    def load_data(self):
        ...
        if self.recording is not None:
            ...
            all_rec_dirs = [
                os.path.join(self.rec_root, f)
                for f in sorted(os.listdir(self.rec_root))
                if f[0] != "."
            ]
            all_rec_dirs = [f for f in all_rec_dirs if os.path.isdir(f)]
            recording_list = [
                f
                for f in all_rec_dirs
                if f.split("/")[-1].split("_")[0] in self.split_scenes
            ]
    ...
    def get_data_paths_from_img(self, img_paths):
        ...
        mask_color_paths = [
            osp.join(mask_color_folder, f + ".png") for f in frame_names
        ]
    ...
    def __getitem__(self, idx):
        ...
        (
            ...
            mask_color_paths,
            fitting_paths,
        ) = self.get_data_paths_from_img(cur_img_paths)
        ...
        if (not self.quant and self.flip) or (self.quant and not self.flip):
            ...
            joint2d_data[:, :, 0] = (
                IMG_WIDTH - joint2d_data[:, :, 0]
            )  # visually reflect about y
        ...
        if self.load_depth:
            ...
            for depth_img, mask in zip(depth_img_list, mask_list):
                scan_dict = self.projection.create_scan(
                    mask, depth_img, mask_on_color=self.mask_color, coord="color"
                )  # want points in RGB camera system
        ...
        if self.load_scene_mesh or self.estimate_floor_plane:
            ...
            scene_mesh = trimesh.load(
                os.path.join(self.scenes_root, scene_name + ".ply")
            )
            ...
            if self.estimate_floor_plane:
                ...
                if neg_points.shape[0] > samp_n_pts:
                    samp_inds = np.random.choice(
                        np.arange(neg_points.shape[0]), size=samp_n_pts, replace=False
                    )
                    neg_points = neg_points[samp_inds]
                    point_heights = point_heights[samp_inds]
                # cluster point heights
                clustering = DBSCAN(eps=0.005, min_samples=100).fit(
                    point_heights.reshape((-1, 1))
                )
                num_clusters = np.unique(clustering.labels_).shape[
                    0
                ...
                max_clust_mean = np.mean(
                    neg_points[clustering.labels_ == max_clust_label], axis=0
                )
                next_clust_label = sort_inds[1]
                next_clust_mean = np.mean(
                    neg_points[clustering.labels_ == next_clust_label], axis=0
                )
                max_clust_idx = (
                    max_clust_label
                    if max_clust_mean[2] <= next_clust_mean[2]
                    else next_clust_label
                )
...
class Projection:
    ...
    def unproject_depth_image(self, depth_image, cam):
        ...
        xyz_camera_space = np.hstack(
            (xy_undistorted_camspace.squeeze(), self.col(uvd[:, 2]))
        )
        xyz_camera_space[:, :2] *= self.col(xyz_camera_space[:, 2])  # scale x,y by z
        other_answer = xyz_camera_space - self.row(
            np.asarray(cam["view_mtx"])[:, 3]
        )  # translate
    ...
    def create_scan(
        ...
        if mask_on_color:
            ...
            valid_mask_idx[valid_mask_idx == True] = (
                mask[uvs[valid_idx == True][:, 1], uvs[valid_idx == True][:, 0]] == 0
            )
        ...
        if coord == "color":
            ...
            T = np.concatenate(
                [
                    np.asarray(self.color_cam["view_mtx"]),
    ...
    def align_color2depth(self, depth_im, color_im, interpolate=True):
        ...
        if interpolate:
            ...
            depth_im_flat[zero_mask] = np.interp(
