    ...
    def load_data(self):
        ...
        if self.scene is not None:
            ...
            all_scene_dirs = [
                ...
                for scene_name in self.split_scenes
            ]
            scene_list = [
                f for f in all_scene_dirs if os.path.exists(f) and os.path.isdir(f)
            ]
        ...
        for scene_path, scene_name in zip(scene_list, scene_names):
            ...
            frame_names = [
                ".".join(f.split("/")[-1].split(".")[:-1]) for f in img_paths
            ]
            ...
            keyp_paths = [
                osp.join(keyp_folder, f + "_keypoints.json") for f in frame_names
            ]
            ...
            #
            scene_info_path = osp.join(
                scene_path, "gt/skel_%s_GT.json" % (SCENE_MAP[scene_name])
            )
            ...
            for k, v in sorted(scene_info["3d"].items()):
                frame_id = int(k)
                pose = np.zeros(
                    shape=(len(v[list(v.keys())[0]]), len(v)), dtype=np.float32
                )
            ...
            joints3d_aligned = np.ones(
                (nobs_frames, ngt_joints, 3), dtype=np.float
            ) * float("inf")
            occlusions_aligned = np.ones(
                (nobs_frames, ngt_joints), dtype=np.float
            ) * float("inf")
            ...
            if self.load_gt_floor or self.load_scene_mesh:
                ...
                scene_json = [
                    os.path.join(scene_path, "gt", f.replace(".obj", ".json"))
                    for f in scene_obj
                ]
                ...
                for part_idx, part_json_path in enumerate(scene_json):
                    obb, data_obb = Obb.load(part_json_path, return_data=True)
                    ...
                    scene_faces.append(
                        part_fs + part_idx * 8
                    )  # there are 8 vertices in each part
                ...
                if self.load_gt_floor:
                    ...
                    est = LinearRegression().fit(
                        floor_fit_pts[:, [0, 2]], floor_fit_pts[:, 1]
                    )
            ...
            if self.scene_subseq_idx > -1:
                ...
                data_out["names"].append(
                    scene_name + "_" + "%04d" % (self.scene_subseq_idx)
                )
...
class Obb(object):
    ...
    def __init__(self, centroid=None, axes=None, scales=None):
        self._centroid = (
            np.asarray(centroid, dtype=np.float32).reshape((3, 1))
            if centroid is not None
            else np.zeros(shape=(3, 1), dtype=np.float32)
        )
        self._axes = (
            axes if axes is not None else np.identity(3, dtype=np.float32)
        )  # axes in cols
        self._scales = (
            np.asarray(scales, dtype=np.float32).reshape((3, 1))
    ...
    @axes.setter
    def axes(self, axes):
        assert axes.shape == (3, 3), "Not allowed shape: %s" % axes.shape.__repr__()
        self._axes = axes
        self._corners_3d = None
    ...
    def axis(self, axis_id):
        assert self._scales.shape == (3, 1), (
            "[SceneObj::axis] wrong _scales shape: %s" % self._scales.shape.__repr__()
        )
        assert self._axes.shape == (3, 3), (
            "[SceneObj::axis] wrong _axes shape: %s" % self._axes.shape.__repr__()
        )
    ...
    def set_axis(self, axis_id, value):
        ...
        assert self._axes.shape == (3, 3), (
            "[sceneObj::set_axis] shape changed: %s" % self._axes.shape.__repr__()
        )
    ...
    def corners_3d(self):
        ...
        if self._corners_3d is not None:
            ...
            for a in range(3):
                half_axes[:, a] = self.axis(axis_id=a) / 2.0
            corners_3d = np.zeros((len(Obb.corners()), 3), np.float32)
    ...
    def corners_3d_lower(self, up_axis=(0.0, -1.0, 0.0)):
    ...
    def faces_3d(self):
        ...
        assert corners_3d.shape[1] == 3, "assumed Nx3: %s" % corners_3d.shape
        faces_3d = np.zeros(shape=(3, 3, len(Obb.face_ids())))
        for face_id, face in enumerate(Obb.face_ids()):
            for d in range(3):
                faces_3d[d, :, face_id] = corners_3d[face[d], :]
        return faces_3d
    ...
    def faces_3d_memoized(self):
        if self._faces_3d is None:
            ...
            for face_id, face in enumerate(Obb.face_ids()):
                for d in range(3):
                    self._faces_3d[d, :, face_id] = corners_3d[face[d], :]
    ...
    def to_obj_string(self, name, vertex_offset=0):
        ...
        for row in range(corners_3d.shape[0]):
            lines.append(
                "v %f %f %f\n"
                % (corners_3d[row, 0], corners_3d[row, 1], corners_3d[row, 2])
            )
    ...
    def __eq__(self, other):
        return (
            np.allclose(self.centroid, other.centroid)
