...
def eval_recon(
    ...
    female_bm_path = os.path.join(SMPLH_PATH, "female/model.npz")
    male_bm = BodyModel(
        bm_path=male_bm_path, num_betas=16, batch_size=test_dataset.sample_num_frames
    ).to(device)
    female_bm = BodyModel(
        bm_path=female_bm_path, num_betas=16, batch_size=test_dataset.sample_num_frames
    ).to(device)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            ...
            global_gt_dict["trans"] += (
                torch.tensor([5.0, 5.0, 0.0])
                .reshape((1, 1, 1, 3))
                .to(global_gt_dict["trans"])
            )
            global_gt_dict["joints"] += (
                torch.tensor([5.0, 5.0, 0.0])
                .reshape((1, 1, 1, 3))
                .expand((1, 1, 22, 3))
                .reshape((1, 1, 1, 66))
                .to(global_gt_dict["joints"])
            )
            ...
            latent_z_seq = posterior_z_out[
                0
            ]  # use mean of the posterior (encoder) output
            ...
            decode_input_dict = {
                k: v[:, 0].clone() for k, v in global_gt_dict.items() if k != "contacts"
            }
