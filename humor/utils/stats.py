...
def dhms(td):
    ...
    s = td.seconds - (
        (h * 3600) + (m * 60)
    )  # td.seconds are the seconds remaining after days have been removed
    return d, h, m, s
...
class StatTracker(object):
    ...
    def update(self, stats_dict, tag="train", save_tf=True, n=1, increment_step=True):
        ...
        if save_tf:
            ...
            for k, v in pcl_dict.items():
                ...
                points_config = {"cls": "PointsMaterial", "size": 0.05}
                self.writer.add_mesh(
                    all_tag + "/" + k,
                    v,
                    colors.to(torch.int),
                    global_step=self.step,
                    config_dict={"material": points_config},
                )
    ...
    def print(
        ...
        progress_str = "[{}>{}] {} epoch - {}/{} | batch - {}/{}".format(
            "=" * done,
            "-" * (50 - done),
            tag,
            cur_epoch_idx + 1,
        ...
        if time_per_batch_str in self.meter_dict and total_elapsed_time is not None:
            ...
            cur_frac = (num_batches * cur_epoch_idx + cur_batch_idx) / (
                num_batches * num_epochs
            )
            ETA = (elapsed / (cur_frac + 1e-6)) - elapsed
            ETA_str = getTimeDur(ETA)
            time_str = "%.3f s per batch | %s elapsed | %s ETA" % (
                mean_per_batch,
                elapsed_str,
                ETA_str,
            )
