...
def save_optim_result(
    ...
    if "stage3" in per_stage_results and optim_floor:
        ...
        res_root_orient = (
            per_stage_results["stage3"]["prior_root_orient"].detach().cpu().numpy()
        )
        for bidx, cur_res_out_path in enumerate(cur_res_out_paths):
            cur_res_out_path = os.path.join(
                cur_res_out_path, "stage3_results_prior.npz"
            )
            save_dict = {
                "betas": res_betas[bidx],
                "trans": res_trans[bidx],
    ...
    save_gt = (
        "betas" in gt_data
        and "trans" in gt_data
        and "root_orient" in gt_data
        and "pose_body" in gt_data
    )
    ...
    if save_gt:
        ...
        if data_type not in ["PROX-RGB", "PROX-RGBD"]:
            gt_betas = gt_betas[
                :, 0
            ]  # only need frame 1 for e.g. 3d data since it's the same over time.
        ...
        for bidx, cur_res_out_path in enumerate(cur_res_out_paths):
            ...
            save_dict = {"joints3d": gt_joints[bidx]}
            if cam_mat is not None:
                save_dict["cam_mtx"] = cam_mat[bidx]
    ...
    for bidx, cur_res_out_path in enumerate(cur_res_out_paths):
        ...
        if obs_mask_paths is not None:
            cur_obs_out["mask_paths"] = [
                frame_tup[bidx] for frame_tup in obs_mask_paths
            ]
        print("Saving observations", cur_obs_out.keys())
        np.savez(obs_out_path, **cur_obs_out)


def save_rgb_stitched_result(
    seq_intervals,
    all_res_out_paths,
    res_out_path,
    device,
    ...
    for res_idx, res_dir in enumerate(all_res_dirs):
        ...
        if concat_ground_planes is None:
            concat_ground_planes = (
                torch.Tensor(cur_stage3_res["floor_plane"]).to(device).reshape((1, -1))
            )
        else:
            concat_ground_planes = torch.cat(
                [
                    concat_ground_planes,
                    torch.Tensor(cur_stage3_res["floor_plane"])
                    .to(device)
                    .reshape((1, -1)),
                ],
                dim=0,
        ...
        cur_stage3_res = {
            k: v
            for k, v in cur_stage3_res.items()
            if k in ["betas", "trans", "root_orient", "pose_body", "world_scale"]
        }
        cur_stage3_res = prep_res(
            cur_stage3_res, device, cur_stage3_res["trans"].shape[0]
        )
        if concat_cam_res is None:
            ...
            for k, v in concat_cam_res.items():
                concat_cam_res[k] = torch.cat(
                    [concat_cam_res[k], cur_stage3_res[k][seq_overlaps[res_idx] :]],
                    dim=0,
                )
            concat_contacts = torch.cat(
                [concat_contacts, cur_contacts[seq_overlaps[res_idx] :]], dim=0
            )
        ...
        if concat_joints2d is None:
            ...
            concat_joints2d = np.concatenate(
                [concat_joints2d, cur_obs["joints2d"][seq_overlaps[res_idx] :]], axis=0
            )
        if concat_img_paths is None:
            concat_img_paths = list(cur_obs["img_paths"])
        else:
            concat_img_paths = concat_img_paths + list(
                cur_obs["img_paths"][seq_overlaps[res_idx] :]
            )
    ...
    np.savez(
        os.path.join(final_res_out_path, "observations.npz"),
        joints2d=concat_joints2d,
        img_paths=concat_img_paths,
    )
    ...
    res_floor_plane = (
        concat_ground_planes[0].clone().detach().cpu().numpy()
    )  # NOTE: saves estimate from first subsequence
    ...
    cam2prior_R, cam2prior_t, cam2prior_root_height = compute_cam2prior(
        concat_ground_planes[0].unsqueeze(0),
    ...
    input_data_dict = {
        kb: vb.unsqueeze(0)
        for kb, vb in concat_cam_res.items()
        if kb in ["trans", "root_orient", "pose_body", "betas", "world_scale"]
    }
    viz_prior_data_dict = apply_cam2prior(
        input_data_dict,
        cam2prior_R,
        cam2prior_t,
        cam2prior_root_height,
    ...
    concat_prior_res_out_path = os.path.join(
        final_res_out_path, "stage3_results_prior.npz"
    )
...
def apply_cam2prior(
    ...
    if "trans" in data_dict and "root_orient" in data_dict:
        ...
        if inverse:
            ...
            trans = torch.matmul(R_time.transpose(3, 2), trans.reshape((B, T, 3, 1)))[
                :, :, :, 0
            ]
            ...
            cur_smpl_body = body_model(
                pose_body=body_pose.reshape((-1, body_pose.size(2))),
                pose_hand=None,
