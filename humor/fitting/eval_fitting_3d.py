...
from fitting.eval_utils import (
    quant_eval_3d,
    SMPL_SIZES,
    GRND_PEN_THRESH_LIST,
    get_grnd_pen_key,
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the results directory to run eval on. Should be a directory of directories - one for each sequence.",
    )
    parser.add_argument(
        "--out",
        type=str,
        required=True,
        help="Path to save evaluation results/visualizations to.",
    )
    ...
    parser.add_argument(
        ...
        dest="run_quant_eval",
        action="store_true",
        help="If given, runs quantitative evaluation and saves the results.",
    )
    ...
    parser.add_argument(
        ...
        dest="quant_stages",
        action="store_true",
        help="If given, runs quantitative evaluation on all stages rather than just final.",
    )
    ...
    parser.add_argument(
        ...
        dest="run_qual_eval",
        action="store_true",
        help="If given, runs qualitative (visualization) evaluation and saves the results. By default this is just the final result and GT.",
    )
    ...
    parser.add_argument(
        ...
        dest="viz_stages",
        action="store_true",
        help="If given, visualized intermediate optimization stages.",
    )
    parser.set_defaults(viz_stages=False)
    parser.add_argument(
        "--viz-observation",
        dest="viz_observation",
        action="store_true",
        help="If given, visualizes observations on bodies (e.g. verts).",
    )
    ...
    parser.add_argument(
        "--shuffle", dest="shuffle", action="store_true", help="Shuffles eval ordering"
    )
...
def main(args):
    ...
    for ridx, result_dir in enumerate(all_result_dirs):
        ...
        if pred_res is None:
            print(
                "Could not find final pred (stage 3) results for %s, skipping..."
                % (seq_name)
            )
            continue
        ...
        if gt_bm_path not in body_model_dict:
            gt_bm = BodyModel(
                bm_path=gt_bm_path, num_betas=num_gt_betas, batch_size=T
            ).to(device)
        ...
        if args.quant_stages or args.viz_stages:
            ...
            for stage_name in STAGES_RES_NAMES:
                ...
                if stage_res is None:
                    print(
                        "Could not find results for stage %s of %s, skipping..."
                        % (stage_name, seq_name)
                    )
                    continue
        ...
        if args.run_quant_eval:
            ...
            if not os.path.exists:
                print(
                    "Could not find observed data for %s, using in quantitative eval..."
                    % (seq_name)
                )
            ...
            for eval_body, eval_name, eval_res in zip(
                eval_bodies, eval_names, eval_res_list
            ):
                ...
                if "contacts" in pred_res:
                    cur_pred_eval["contacts"] = pred_res[
                        "contacts"
                    ]  # stages don't have contacts so just always use pred
                else:
                    # didn't predict contacts, just use Gt
                    cur_pred_eval["contacts"] = gt_res["contacts"]
                quant_eval_3d(
                    quant_res_dict[eval_name], cur_pred_eval, cur_gt_eval, obs_data
                )
        ...
        if args.run_qual_eval:
            ...
            if args.viz_observation:
                ...
                if not os.path.exists:
                    print(
                        "Could not find observed data for %s, not visualizing..."
                        % (seq_name)
                    )
                else:
                    obs_dict = np.load(obs_path)
                    if "verts3d" in obs_dict.files:
                        obs_data["viz_points"] = torch.Tensor(obs_dict["verts3d"]).to(
                            device
                        )
                    ...
                    if "joints3d" in obs_dict.files:
                        obs_data["viz_joints"] = torch.Tensor(obs_dict["joints3d"]).to(
                            device
                        )

                    if obs_data["viz_joints"] is not None:
                        obs_data["viz_joints"][
                            torch.isinf(obs_data["viz_joints"])
                        ] = -1.0  # out of sight
                    if obs_data["viz_points"] is not None:
                        obs_data["viz_points"][
                            torch.isinf(obs_data["viz_points"])
                        ] = -1.0  # out of sight

            viz_contacts = pred_res["contacts"] if args.viz_contacts else None
            gt_contacts = (
                gt_res["contacts"]
                if "contacts" in gt_res and args.viz_contacts
                else None
            )
            ...
            create_video(
                static_out_path + "/frame_%08d." + "%s" % ("png"),
                static_out_path + ".mp4",
                FPS,
            )
    ...
    if args.run_quant_eval:
        ...
        supp_vals = [
            "ground_pen_dist_normalized",
            "ground_pen_mean_agg_frac",
        ]  # values computed after from everything else
        ...
        for eval_name, eval_res in quant_res_dict.items():
            ...
            for frac_val in frac_vals:
                ...
                per_seq_means = [
                    float(seq_val) / seq_cnt
                    for seq_val, seq_cnt in zip(per_seq_val, per_seq_cnt)
                ]
                agg_per_seq_means.append(per_seq_means)
            ...
            # supplemental values
            ground_pen_dist_normalized_mean = (
                agg_all_means[out_vals.index("ground_pen_dist")]
                * agg_all_means[out_vals.index(get_grnd_pen_key(0.0))]
            )
            agg_all_means.append(ground_pen_dist_normalized_mean)
            ground_pen_dist_normalized_med = (
                agg_all_meds[out_vals.index("ground_pen_dist")]
                * agg_all_means[out_vals.index(get_grnd_pen_key(0.0))]
            )
            agg_all_meds.append(ground_pen_dist_normalized_med)
            ground_pen_frac_sum = 0.0
            for pen_thresh in GRND_PEN_THRESH_LIST:
                ground_pen_frac_sum += agg_all_means[
                    out_vals.index(get_grnd_pen_key(pen_thresh))
                ]
            ...
            stage_out_path = os.path.join(
                quant_out_path, eval_name + "_per_seq_mean.csv"
            )
            with open(stage_out_path, "w") as f:
                ...
                for j, seq_name in enumerate(seq_name_list):
                    cur_row = [
                        agg_per_seq_means[vidx][j]
                        for vidx in range(len(per_seq_out_vals))
                    ]
            ...
            for stat_name, stat_data in zip(stats_name_list, stats_list):
                agg_out_path = os.path.join(
                    quant_out_path, eval_name + "_agg_%s.csv" % (stat_name)
                )
        ...
        for stat_name, stat_data in zip(stats_name_list, stats_list):
            compare_out_path = os.path.join(
                quant_out_path, "compare_%s.csv" % (stat_name)
            )
