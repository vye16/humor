...
RGB_EVAL_BLACKLIST = ["MPH1Library_00145_01_0031", "N0Sofa_03403_01_0013"]
...
def quant_eval_3d(eval_dict, pred_data, gt_data, obs_data):
    ...
    for modality in ["joints3d", "verts3d", "mesh3d"]:
        ...
        if modality == "joints3d":
            joints3d_ee = (
                compute_subset_smpl_joint_err(eval_pred, eval_gt, subset="ee")
                .detach()
                .cpu()
                .numpy()
            )
            joints3d_legs = (
                compute_subset_smpl_joint_err(eval_pred, eval_gt, subset="legs")
                .detach()
                .cpu()
                .numpy()
            )
        ...
        if modality in obs_data:
            ...
            if num_invis_pts > 0:
                ...
                invis_err = (
                    torch.norm(pred_invis - gt_invis, dim=-1).detach().cpu().numpy()
                )
    ...
    floor_plane[2] = 1.0
    num_pen_list, num_tot, pen_dist = compute_toe_floor_pen(
        pred_data["joints3d"], floor_plane, thresh_list=GRND_PEN_THRESH_LIST
    )
    ...
    pred_contacts = pred_data["contacts"][
        :, CONTACT_INDS
    ]  # only compare joints for which the prior is trained
...
def quant_eval_2d(
    ...
    do_comparison = pred_joints_comp is not None and gt_joints_comp is not None
    ...
    if do_comparison:
        ...
        eval_pred_joints = pred_joints_comp[valid_frame_mask].reshape(
            (num_val_frames, J, 3)
        )
        eval_gt_joints = gt_joints_comp[valid_frame_mask].reshape(
            (num_val_frames, J, 3)
        )

        # all joint errors
        joints3d_all = (
            torch.norm(eval_pred_joints - eval_gt_joints, dim=-1).detach().cpu().numpy()
        )
        eval_dict["joints3d_all"].append(joints3d_all)

        # end-effector and related errors
        joints3d_ee = (
            compute_subset_comp_joint_err(eval_pred_joints, eval_gt_joints, subset="ee")
            .detach()
            .cpu()
            .numpy()
        )
        joints3d_legs = (
            compute_subset_comp_joint_err(
        ...
        joints3d_align_all = (
            torch.norm(eval_pred_joints_align - eval_gt_joints_align, dim=-1)
            .detach()
            .cpu()
            .numpy()
        )
        ...
        joints3d_align_ee = (
            compute_subset_comp_joint_err(
                eval_pred_joints_align, eval_gt_joints_align, subset="ee"
            )
            .detach()
            .cpu()
            .numpy()
        ...
        joints3d_align_legs = (
            compute_subset_comp_joint_err(
                eval_pred_joints_align, eval_gt_joints_align, subset="legs"
            )
            .detach()
            .cpu()
            .numpy()
        ...
        if vis_mask is not None and cam_intrins is not None:
            ...
            cam_R = (
                torch.eye(3).reshape((1, 3, 3)).expand((Tv, 3, 3)).to(eval_gt_joints)
            )
            ...
            gt_joints2d = perspective_projection(
                eval_gt_joints, cam_R, cam_t, cam_f, cam_cent
            )
            ...
            occlusion_mask = (
                torch.Tensor(occlusion_mask).to(torch.bool).to(eval_gt_joints.device)
            )
            ...
            if num_vis_pts > 0:
                ...
                joints3d_vis = (
                    torch.norm(eval_pred_vis - eval_gt_vis, dim=-1)
                    .detach()
                    .cpu()
                    .numpy()
                )
            ...
            if num_occl_pts > 0:
                eval_pred_occl = eval_pred_joints[occlusion_mask].reshape(
                    (num_occl_pts, 3)
                )
                eval_gt_occl = eval_gt_joints[occlusion_mask].reshape((num_occl_pts, 3))
                joints3d_occl = (
                    torch.norm(eval_pred_occl - eval_gt_occl, dim=-1)
                    .detach()
                    .cpu()
                    .numpy()
                )
            ...
            # visible absolute data
            if num_vis_pts > 0:
                eval_pred_align_vis = eval_pred_joints_align[vis_mask].reshape(
                    (num_vis_pts, 3)
                )
                eval_gt_align_vis = eval_gt_joints_align[vis_mask].reshape(
                    (num_vis_pts, 3)
                )
                joints3d_align_vis = (
                    torch.norm(eval_pred_align_vis - eval_gt_align_vis, dim=-1)
                    .detach()
                    .cpu()
                    .numpy()
                )
            ...
            # occluded absolute data
            if num_occl_pts > 0:
                eval_pred_align_occl = eval_pred_joints_align[occlusion_mask].reshape(
                    (num_occl_pts, 3)
                )
                eval_gt_align_occl = eval_gt_joints_align[occlusion_mask].reshape(
                    (num_occl_pts, 3)
                )
                joints3d_align_occl = (
                    torch.norm(eval_pred_align_occl - eval_gt_align_occl, dim=-1)
                    .detach()
                    .cpu()
                    .numpy()
                )
    ...
    eval_dict["accel_mag_align"].append(
        pred_joint_align_accel_mag.detach().cpu().numpy()
    )

    # toe-floor penetration
    num_pen_list, num_tot, pen_dist = compute_toe_floor_pen(
        pred_joints_smpl, floor_plane, thresh_list=GRND_PEN_THRESH_LIST
    )
    eval_dict["ground_pen_dist"].append(pen_dist.detach().cpu().numpy())
    for thresh_idx, pen_thresh in enumerate(GRND_PEN_THRESH_LIST):
        cur_pen_key = get_grnd_pen_key(pen_thresh)
...
def compute_subset_smpl_joint_err(eval_pred_joints, eval_gt_joints, subset="ee"):
    ...
    if subset == "ee":
        subset_inds = [
            SMPL_JOINTS["leftFoot"],
            SMPL_JOINTS["rightFoot"],
            SMPL_JOINTS["leftToeBase"],
            SMPL_JOINTS["rightToeBase"],
            SMPL_JOINTS["leftHand"],
            SMPL_JOINTS["rightHand"],
        ]
    elif subset == "legs":
        subset_inds = [
            SMPL_JOINTS["leftFoot"],
    ...
    joint_err = torch.norm(
        eval_pred_joints[:, subset_inds] - eval_gt_joints[:, subset_inds], dim=-1
    )
    return joint_err
...
def compute_toe_floor_pen(joint_seq, floor_plane, thresh_list=[0.0]):
    ...
    _, s = compute_plane_intersection(
        toe_joints,
        -floor_normal,
        floor_plane.reshape((1, 4)).expand((toe_joints.size(0), 4)),
    )
...
SMPL2COMPARE = [
    ...
    SMPL_JOINTS["neck"],
    SMPL_JOINTS["rightHand"],
    SMPL_JOINTS["rightForeArm"],
    SMPL_JOINTS["rightArm"],
    SMPL_JOINTS["leftArm"],
    SMPL_JOINTS["leftForeArm"],
    SMPL_JOINTS["leftHand"],
]
