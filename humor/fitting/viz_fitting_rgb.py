...
from viz.utils import (
    ...
    create_gif,
    create_video,
    create_multi_comparison_images,
    smpl_connections,
    imapper_connections,
    comp_connections,
)
...
STAGES_RES_NAMES = [
    ...
    "stage2_results",
    "stage3_init_results",
]  # results in camera frame
STAGES_PRIOR_RES_NAMES = [
    "stage2_results_prior",
    "stage3_init_results_prior",
]  # results in prior frame (w.r.t final floor fit)
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the results_out directory from fitting to run viz on.",
    )
    parser.add_argument(
        "--out", type=str, required=True, help="Path to save visualizations to."
    )
    ...
    parser.add_argument(
        "--viz-no-render-cam-body",
        dest="viz_render_cam_body",
        action="store_false",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1280,
        help="Width of rendered output images",
    )
    parser.add_argument(
        "--viz-render-height",
        type=int,
    ...
    parser.add_argument(
        "--shuffle", dest="shuffle", action="store_true", help="Shuffles viz ordering"
    )
...
def main(args):
    ...
    for residx, result_dir in enumerate(all_result_dirs):
        ...
        cur_frame_names = [
            ".".join(f.split("/")[-1].split(".")[:-1]) for f in cur_img_paths
        ]
            ...
            if not cur_valid:
                ...
                if smpk == "betas":
                    ...
                    pred_res[smpk] = np.zeros(
                        (T, pred_res[smpk].shape[1]), dtype=np.float
                    )
        floor_valid = (
            torch.sum(
                torch.logical_not(torch.isfinite(torch.Tensor(pred_res["floor_plane"])))
            ).item()
            == 0
        )
        if not floor_valid:
        ...
        if args.viz_prior_frame:
            ...
            if pred_res_prior is None:
                print(
                    "Could not find final prior pred (stage 3) results for %s, skipping..."
                    % (seq_name)
                )
                continue
        ...
        for fidx, img in enumerate(img_arr):
            ...
            cv2.imwrite(
                os.path.join(og_out_path, "frame_%08d.%s" % (fidx, IM_EXTN)),
                img_bgr,
                COMPRESS_PARAMS,
            )
        create_video(
            og_out_path + "/frame_%08d." + "%s" % (IM_EXTN), og_out_path + ".mp4", FPS
        )
        ...
        if args.viz_obs_2d:
            ...
            fig = plt.figure(
                figsize=(float(D_IMW) * 0.01, float(D_IMH) * 0.01),
                dpi=100,
                frameon=False,
            )
            for fidx, img in enumerate(img_arr):
                ...
                plt.scatter(
                    viz_joints2d[fidx, valid_mask, 0],
                    viz_joints2d[fidx, valid_mask, 1],
                    c="lime",
                    s=100,
                )
                ax = plt.gca()
                plt.axis("off")
                cur_joint2d_out = os.path.join(
                    og_2d_obs_out_path, "frame_%08d.png" % (fidx)
                )
            ...
            create_video(
                og_2d_obs_out_path + "/frame_%08d." + "%s" % ("png"),
                og_2d_obs_out_path + ".mp4",
                FPS,
            )
        ...
        create_video(
            pred_out_path + "/frame_%08d." + "%s" % (IM_EXTN),
            pred_out_path + ".mp4",
            FPS,
        )
        ...
        create_multi_comparison_images(
            [og_out_path, pred_out_path],
            og_pred_comp_path,
            ["Input", "Final"],
            extn=IM_EXTN,
        )
        create_video(
            og_pred_comp_path + "/frame_%08d." + "%s" % (IM_EXTN),
            og_pred_comp_path + ".mp4",
            FPS,
        )
        ...
        if cur_viz_stages:
            for k, stage_body in stages_body.items():
                ...
                create_video(
                    stage_out_path + "/frame_%08d." + "%s" % (IM_EXTN),
                    stage_out_path + ".mp4",
                    FPS,
                )
            ...
            if cur_viz_stages:
                pred_stage_comp_path = os.path.join(
                    cur_qual_out_path, "comp_final_pred_stages"
                )
                create_multi_comparison_images(
                    [og_out_path, stage2_out_path, pred_out_path],
                    pred_stage_comp_path,
                    ["Input", "Stage2", "Final"],
                    extn=IM_EXTN,
                )
                create_video(
                    pred_stage_comp_path + "/frame_%08d." + "%s" % (IM_EXTN),
                    pred_stage_comp_path + ".mp4",
                    FPS,
                )
        ...
        if args.viz_prior_frame:
            ...
            create_video(
                pred_prior_out_path + "/frame_%08d." + "%s" % (IM_EXTN),
                pred_prior_out_path + ".mp4",
                FPS,
            )

            # always comparison of prediction and OG video
            og_pred_prior_comp_path = os.path.join(
                cur_qual_out_path, "comp_og_final_pred_prior"
            )
            create_multi_comparison_images(
                [og_out_path, pred_out_path, pred_prior_out_path],
                og_pred_prior_comp_path,
                ["Input", "FinalCam", "FinalPrior"],
                extn=IM_EXTN,
            )
            create_video(
                og_pred_prior_comp_path + "/frame_%08d." + "%s" % (IM_EXTN),
                og_pred_prior_comp_path + ".mp4",
                FPS,
            )
            ...
            if cur_viz_stages:
                for k, stage_prior_body in stages_prior_body.items():
                    ...
                    create_video(
                        stage_prior_out_path + "/frame_%08d." + "%s" % (IM_EXTN),
                        stage_prior_out_path + ".mp4",
                        FPS,
                    )

                # create comparison
                stage2_prior_out_path = os.path.join(
                    cur_qual_out_path, STAGES_PRIOR_RES_NAMES[0]
                )
                if cur_viz_stages:
                    pred_stage_prior_comp_path = os.path.join(
                        cur_qual_out_path, "comp_final_pred_prior_stages"
                    )
                    create_multi_comparison_images(
                        [og_out_path, stage2_prior_out_path, pred_prior_out_path],
                        pred_stage_prior_comp_path,
                        ["Input", "Stage2Prior", "FinalPrior"],
                        extn=IM_EXTN,
                    )
                    create_video(
                        pred_stage_prior_comp_path + "/frame_%08d." + "%s" % (IM_EXTN),
                        pred_stage_prior_comp_path + ".mp4",
                        FPS,
                    )
