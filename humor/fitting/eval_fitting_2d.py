...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["iMapper", "PROX", "PROXD"],
        help="The dataset we are evaluating results on",
    )
    parser.add_argument(
        "--prox-floors",
        type=str,
        default=None,
        help="Path to the GT floor plane data for PROX. If given, then the GT floor is used for plausibility metrics rather than predicted floor from our method.",
    )
    parser.add_argument(
        "--imapper-floors",
        type=str,
        default=None,
        help="Path to the GT floor plane data for iMapper. If given, then the GT floor is used for plausibility metrics rather than predicted floor from our method.",
    )
    ...
    parser.add_argument(
        ...
        dest="quant_stages",
        action="store_true",
        help="If given, runs quantitative evaluation on all stages in addition to the final results.",
    )
...
def main(args):
    ...
    for residx, result_dir in enumerate(all_result_dirs):
        ...
        scene_name = "_".join(
            seq_name.split("_")[:-1]
        )  # the full sequence before splitting up
        print(scene_name)
        ...
        if args.dataset in ["PROX", "PROXD"] and args.prox_floors is not None:
            ...
            if not os.path.exists(floor_path):
                print(
                    "Could not find floor plane for scene at %s...using predicted"
                    % (floor_path)
                )
            else:
                print(
                    "Using GT floors from PROX dataset for plausibility metrics rather than predicted!"
                )
            ...
            floor_path = os.path.join(
                args.imapper_floors, cur_imapper_scene_name + ".txt"
            )
            if not os.path.exists(floor_path):
                print(
                    "Could not find floor plane for scene at %s...using predicted"
                ...
                print(
                    "Using GT floors from iMapper dataset for plausibility metrics rather than predicted!"
                )
                with open(floor_path, "r") as floor_f:
                    floor_str = floor_f.readline()
                floor_arr = [float(x) for x in floor_str.split(" ")]
        ...
        eval_floor_plane = (
            gt_floor_plane if gt_floor_plane is not None else pred_floor_plane
        )
    ...
    for eval_name, eval_res in quant_res_dict.items():
        ...
        with open(stage_out_path, "w") as f:
            ...
            for j, seq_name in enumerate(seq_name_list):
                cur_row = [
                    agg_per_seq_means[vidx][j] for vidx in range(len(per_seq_out_vals))
                ]
