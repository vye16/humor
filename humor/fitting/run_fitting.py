...
def main(args, config_file, extra_args={}):
    ...
    if args.data_type == "AMASS":
        ...
        if not args.data_is_dir:
            ...
            if not os.path.exists(img_folder):
                ...
                img_folder = video_to_images(
                    args.data_path, fps=30, img_folder=img_folder
                )
            ...
            camera_path = os.path.join(
                args.data_path, "cameras", args.video_seq, "cameras.npz"
            )
            print(img_folder, camera_path)
            if args.op_keypts is not None:
                assert args.track_id is not None
                args.op_keypts = os.path.join(
                    args.data_path, args.op_keypts, args.video_seq, args.track_id
                )
                assert os.path.isdir(args.op_keypts)
            if args.mask_joints2d:
                assert args.track_id is not None
                mask_out_path = os.path.join(
                    args.data_path, args.mask_dir, args.video_seq, args.track_id
                )
        ...
        if use_custom_keypts:
            assert os.path.exists(
                args.op_keypts
            ), "Could not find custom keypoints path %s!" % (args.op_keypts)
    ...
    for i, data in enumerate(data_loader):
        ...
        if (
            cur_res_out_paths is not None
            and args.data_type == "RGB"
            and args.save_results
        ):
        ...
        if "gender" in gt_data:
            ...
            for cur_gender in gt_data["gender"]:
                ...
                if args.gt_body_type == "smplh":
                    gt_body_path = os.path.join(
                        SMPLH_PATH, "%s/model.npz" % (cur_gender)
                    )
        ...
        try:
            ...
            if use_overlap_loss:
                ...
                prev_batch_overlap_res_dict["seq_interval"] = (
                    observed_data["seq_interval"][-1].clone().detach()
                )
...
if __name__ == "__main__":
    ...
    extra_args = {
        x[0].replace("-", "_"): (x[1:] if len(x) > 2 else x[1])
        for x in tokens
        if len(x) > 1
    }
    print(extra_args)
