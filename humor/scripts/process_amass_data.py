...
SAVE_ALIGN_ROT = (
    True  # save rot mats that go from world root orient to aligned root orient
)
...
ROOT_HEIGHT_THRESH = (
    0.04  # if maximum "static" root height is more than this + root_floor_height
)
CLUSTER_SIZE_THRESH = (
    0.25  # if cluster has more than this faction of fps (30 for 120 fps)
)
...
def get_body_model_sequence(
    smplh_path, gender, num_frames, pose_body, pose_hand, betas, root_orient, trans
):
    gender = str(gender)
...
def compute_align_mats(root_orient):
    ...
    root_orient_mat = (
        batch_rodrigues(torch.Tensor(root_orient).reshape(-1, 3))
        .numpy()
        .reshape((num_frames, 9))
    )
    ...
    body_right = -root_orient_mat.reshape((num_frames, 3, 3))[
        :, :, 0
...
def compute_align_from_right(body_right):
    ...
    world2aligned_mat = batch_rodrigues(
        torch.Tensor(world2aligned_aa).reshape(-1, 3)
    ).numpy()
...
def process_seq(data_paths):
    ...
    if input_file_path.find("BMLhandball") >= 0:
        fps = 240
    if (
        input_file_path.find("20160930_50032") >= 0
        or input_file_path.find("20161014_50033") >= 0
    ):
        fps = 59
    ...
    while process_inds[0] < num_frames:
        ...
        body = get_body_model_sequence(
            smplh_path,
            gender,
            process_inds[1] - process_inds[0],
            pose_body[sidx:eidx],
            pose_hand[sidx:eidx],
            betas,
            root_orient[sidx:eidx],
            trans[sidx:eidx],
        )
    ...
    joint_vel_seq = (
        vtx_vel_seq
    ) = trans_vel_seq = root_orient_vel_seq = pose_body_vel_seq = None
    if SAVE_VELOCITIES:
        h = 1.0 / fps
        # joints
        ...
        # root orient
        root_orient_mat = axisangle2matrots(
            root_orient.reshape(num_frames, 1, 3)
        ).reshape((num_frames, 3, 3))
        root_orient_vel_seq = estimate_angular_velocity(root_orient_mat, h)
        # body pose
        pose_body_mat = axisangle2matrots(
            pose_body.reshape(num_frames, len(SMPL_JOINTS) - 1, 3)
        ).reshape((num_frames, len(SMPL_JOINTS) - 1, 3, 3))
    ...
    output_file_path = output_file_path[:-4] + "_%d_frames_%d_fps.npz" % (
        num_frames,
        int(fps),
    )
    np.savez(
...
def main(config):
    ...
    all_dataset_dirs = [
        os.path.join(config.amass_root, f)
        for f in sorted(os.listdir(config.amass_root))
        if f[0] != "."
    ]
    all_dataset_dirs = [f for f in all_dataset_dirs if os.path.isdir(f)]
    print(
        "Found %d available datasets from raw AMASS data source."
    ...
    for data_dir, data_name in zip(dataset_dirs, dataset_names):
        if not os.path.exists(data_dir):
            print(
                "Could not find dataset %s in available raw AMASS data!" % (data_name)
            )
            return
        ...
        cur_subject_dirs = [
            ...
            if f[0] != "." and os.path.isdir(os.path.join(data_dir, f))
        ]
        ...
        already_processed = [
            ...
            if len(glob.glob(output_seqs[i][:-4] + "*.npz")) == 1
        ]
        already_processed_output_names = [
            output_file_names[i] for i in already_processed
        ]
        ...
        not_already_processed = [
            i
            for i in range(len(output_seqs))
            if len(glob.glob(output_seqs[i][:-4] + "*.npz")) == 0
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--amass-root",
        type=str,
        default="./data/amass_raw",
        help="Root directory of raw AMASS dataset.",
    )
    parser.add_argument(
        "--datasets",
        type=str,
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="./data/amass_processed",
        help="Root directory to save processed output to.",
    )
    parser.add_argument(
        "--smplh-root",
        type=str,
        default="./body_models/smplh",
