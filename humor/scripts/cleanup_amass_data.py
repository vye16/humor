...
def main(args):
    ...
    if BML_NTroje:
        ...
        if not os.path.exists(dataset_path):
            print(
                "Could not find BioMotionLab_NTroje data, not filtering out treadmill sequences..."
            )
        else:
    ...
    if MPI_HDM05:
        ...
        if not os.path.exists(dataset_path):
            print(
                "Could not find MPI_HDM05 data, not filtering out inline skating sequences..."
            )
        else:
            # create output directory to backup moved data
            if not os.path.exists(args.backup):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, required=True, help="Root dir of processed AMASS data"
    )
    parser.add_argument(
        "--backup",
        type=str,
