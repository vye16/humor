...
def parse_args(argv):
    ...
    parser.add_argument(
        "--data", type=str, required=True, help="Path to the full AMASS dataset"
    )
    parser.add_argument(
        "--out", type=str, required=True, help="Path to save outputs to"
    )
    parser.add_argument(
        "--train-states",
        type=str,
        default=None,
        help="npy file with pre-loaded train states",
    )
    parser.add_argument(
        "--gmm-comps", type=int, default=12, help="Number of GMM components to use"
    )
    ...
    parser.add_argument(
        ...
        dest="viz_only",
        action="store_true",
        help="If given, only visualizes results in the given output directory, does not refit.",
    )
    ...
    parser.add_argument(
        ...
        dest="test_only",
        action="store_true",
        help="If given, only runs fitting in the given output directory on test data, does not refit.",
    )
...
def main(args):
    ...
    if args.train_states is not None:
        ...
        amass_dataset = AmassDiscreteDataset(
            split="train",
        ...
        for i, data in enumerate(loader):
            ...
            cur_state = torch.cat(
                [joints, joints_vel, trans_vel, root_orient_vel], dim=-1
            )
...
def build_pytorch_gmm(gmm_weights, gmm_means, gmm_covs):
    ...
    comp = MultivariateNormal(
        torch.from_numpy(gmm_means), covariance_matrix=torch.from_numpy(gmm_covs)
    )
...
def test_results(data_path, gmm_path):
    ...
    test_dataset = AmassDiscreteDataset(
        split="test",
